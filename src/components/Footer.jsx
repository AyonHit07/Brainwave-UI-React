            ...
            <ul className='flex flex-wrap gap-5'>
                {socials.map((item)=>(
                    <a key={item.id} href={item.url} target='_blank'>
                        <img src={item.iconUrl} width={16} height={16} alt={item.title} className='flex items-center justify-center w-10 h-10 transition-colors rounded-full bg-n-7 hover:bg-n-6' />
                    </a>
                ))}
            </ul>
        </div>
